...
export default function PromoBanner() {
  ...
  const bannerTitle =
    lang === "ID"
      ? activeBanner.title_id || ""
      : activeBanner.title_en || "";

  const bannerDesc =
    lang === "ID"
      ? activeBanner.description_id || ""
      : activeBanner.description_en || "";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 items-center min-h-80 sm:min-h-96">
          <div className="p-6 sm:p-10 md:p-12 flex flex-col justify-center order-2 md:order-1 z-10">
            <span className="inline-block px-3 py-1 mb-3 text-xs font-semibold tracking-wider text-[#e88900] uppercase bg-[#e88900]/10 border border-[#e88900]/30 rounded-full w-fit">
              {lang === "ID" ? "Promo Spesial" : "Special Promo"}
            </span>
      ...
      {banners.length > 1 && (
        <>
          <button
            ...
            aria-label={lang === "ID" ? "Banner sebelumnya" : "Previous banner"}
          ...
          <button
            type="button"
            onClick={handleNext}
            aria-label={lang === "ID" ? "Banner selanjutnya" : "Next banner"}
